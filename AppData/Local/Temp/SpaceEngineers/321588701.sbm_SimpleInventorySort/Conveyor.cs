...
//							Logging.Instance.WriteLine(string.Format("Conveyor Functional: {0} - {1} - {2} - {3} - {4} - {5}", slimBlockEnd.FatBlock.IsFunctional, slimBlockEnd.FatBlock.IsWorking, IsFunctional(slimBlockEnd.FatBlock), slimBlockEnd.BuildIntegrity, slimBlockEnd.BuildLevelRatio, slimBlockEnd.MaxIntegrity));
		...
		private static bool IsFunctional(IMyCubeBlock block)
		{
            ...
            //Logging.Instance.WriteLine(string.Format("IsFunctional: {0}/{2} - {1}", block.GetObjectBuilderCubeBlock().BuildPercent, def.CriticalIntegrityRatio, block.GetObjectBuilderCubeBlock().IntegrityPercent));
            //return block.GetObjectBuilderCubeBlock().BuildPercent > def.CriticalIntegrityRatio;			
            return false;
		}
    }
}
