namespace Economy.scripts.Management
{
    ...
    public class SeTextBuilder
    {
        ...
        public SeTextBuilder() : this(false, 1.0f)
        {
        }

        public SeTextBuilder(bool isWide, float fontSize)
        {
            _isWide = isWide;
            _fontSize = fontSize;
            _widthModifier =(_isWide ? 2f : 1f) / _fontSize; 
        ...
        {
            _string.AppendFormat(text, args);
        }
        ...
        {
            _string.AppendLine(StringFormatter(text, args));
        }

        public void AddLeftTrim(float desiredWidth, string text, params object[] args)
        {
            _string.Append(GetStringTrimmed(desiredWidth * _widthModifier, StringFormatter(text, args)));
        }

        public void AddRightText(float rightEdgePosition, string text, params object[] args)
        {
            AddRightAlign(_string, rightEdgePosition, StringFormatter(text, args));
        }

        public void AddRightLine(float rightEdgePosition, string text, params object[] args)
        {
            AddRightAlign(_string, rightEdgePosition, StringFormatter(text, args));
            _string.AppendLine();
        }

        public void AddCenterText(float centerPosition, string text, params object[] args)
        {
            AddCenterAlign(_string, centerPosition, StringFormatter(text, args));
        }

        public void AddCenterLine(float centerPosition, string text, params object[] args)
        {
            AddCenterAlign(_string, centerPosition, StringFormatter(text, args));
            _string.AppendLine();
        }

        public void AddFill(string left, char fill, string right)
        {
            AddFillText(_string, left, fill, right);
            _string.AppendLine();
        }
        ...
        private void AddRightAlign(StringBuilder stringBuilder, float rightEdgePosition, string text)
        {
            ...
            rightEdgePosition *= _widthModifier;
        ...
        private void AddFillText(StringBuilder stringBuilder, string left, char fill, string right)
        {
            ...
            var fillSpace = (LcdLineWidth - curWidth) * _widthModifier;
