namespace SpaceEquipmentLtd.NanobotBuildAndRepairSystem
{
   ...
   public class NanobotBuildAndRepairSystemBlock : MyGameLogicComponent
   {
      ...
      {
         NotReady = 0, Idle = 1, Welding = 2, NeedWelding = 3, MissingComponents = 4, Grinding = 5, NeedGrinding = 6, InventoryFull = 7
      }

      public static readonly int WELDER_RANGE_DEFAULT_IN_M = 75;
      ...
      private static MySoundPair[] _Sounds = new [] { null, new MySoundPair("ToolPlayWeldIdle"), new MySoundPair("ToolLrgWeldMetal"), new MySoundPair("BlockModuleProductivity"), new MySoundPair("BlockModuleProductivity"), new MySoundPair("ToolLrgGrindMetal"), new MySoundPair("BlockModuleProductivity"), new MySoundPair("BlockModuleProductivity") };
      ...
      {
         WelderMaximumRange = (int)Math.Ceiling(NanobotBuildAndRepairSystemMod.Settings.Range / (_Welder.BlockDefinition.SubtypeName.Contains("Large") ? 1f : 3f));
         WelderMaximumRequiredElectricPowerTransport = NanobotBuildAndRepairSystemMod.Settings.MaximumRequiredElectricPowerTransport / (_Welder.BlockDefinition.SubtypeName.Contains("Large") ? 1f : 3f);
         WelderMaximumRequiredElectricPowerWelding = NanobotBuildAndRepairSystemMod.Settings.Welder.MaximumRequiredElectricPowerWelding / (_Welder.BlockDefinition.SubtypeName.Contains("Large") ? 1f : 3f);
         WelderMaximumRequiredElectricPowerGrinding = NanobotBuildAndRepairSystemMod.Settings.Welder.MaximumRequiredElectricPowerGrinding / (_Welder.BlockDefinition.SubtypeName.Contains("Large") ? 1f : 3f);
      ...
      private void ServerTryWeldingGrinding()
      {
         ...
         if (missingComponentsChanged || possibleWeldTargetsChanged || possibleGrindTargetsChanged || readyChanged || inventoryFullChanged)
         {
            UpdateCustomInfo();
         }
      }
      ...
      private bool ServerDoGrind(TargetBlockData targetData, out bool transporting)
      {
         ...
         if (_LastTransportStartTime.Milliseconds > TimeSpan.Zero.Milliseconds)
         {
            //Transport started
            if (!ServerEmptyTranportInventory(true))
            {
               if (Mod.Log.ShouldLog(Logging.Level.Info)) Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerDoGrind: Target {1} transport still running transport inventory not emtpy",
                  Logging.BlockName(_Welder, Logging.BlockNameOptions.None), Logging.BlockName(targetData.Block));
               transporting = true;
            ...
            _LastTransportStartTime = TimeSpan.Zero;
         }
         ...
         var grinderAmount = damage;
         if (target.FatBlock != null)
            grinderAmount /= target.FatBlock.DisassembleRatio;
         else
            grinderAmount /= ((MyCubeBlockDefinition)target.BlockDefinition).DisassembleRatio;
         grinderAmount = grinderAmount * ((MyCubeBlockDefinition)target.BlockDefinition).IntegrityPointsPerSec;

         var integrityRatio = (target.Integrity - grinderAmount) / target.MaxIntegrity;
         var emptying = false;
         ...
         if ((float)_TransportInventory.CurrentVolume >= _MaxTransportVolume || target.IsFullyDismounted)
         {
            ...
            if (Mod.Log.ShouldLog(Logging.Level.Info)) Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerDoGrind: Target {1} transport started transporttime={2} CurrentVolume={3}/{4}",
               Logging.BlockName(_Welder, Logging.BlockNameOptions.None), Logging.BlockName(targetData.Block), _TransportTime, _TransportInventory.CurrentVolume, _MaxTransportVolume);
      ...
      private bool ServerEmptyTranportInventory(bool push)
      {
         if (!_TransportInventory.Empty())
         {
            if (!MyAPIGateway.Session.CreativeMode)
            {
               ...
               if (welderInventory != null)
               {
                  ...
                  {
                     MyFixedPoint movedAmount = 0;
                     var id = item.Content.GetId();
                     //Try to move all
                     if (welderInventory.CanItemsBeAdded(item.Amount, id)){
                        welderInventory.AddItems(item.Amount, (MyObjectBuilder_PhysicalObject)MyObjectBuilderSerializer.CreateNewObject(id));
                        _TransportInventory.RemoveItems(item.ItemId, item.Amount, spawn:false);
                        movedAmount = item.Amount;
                     } else
                     {
                        //Try to move as much as possible
                        while (item.Amount > 1)
                        {
                           if (welderInventory.CanItemsBeAdded(1, id))
                           {
                              welderInventory.AddItems(1, (MyObjectBuilder_PhysicalObject)MyObjectBuilderSerializer.CreateNewObject(id));
                              _TransportInventory.RemoveItems(item.ItemId, 1, spawn: false);
                              movedAmount += 1;
                           }
                           else break;
                        }
                     ...
                     if (movedAmount > 0 && Mod.Log.ShouldLog(Logging.Level.Info)) Mod.Log.Write(Logging.Level.Info, "BuildAndRepairSystemBlock {0}: ServerEmptyTranportInventory move to welder Item {1} amount={2}", Logging.BlockName(_Welder, Logging.BlockNameOptions.None), id, movedAmount);
         ...
         return _TransportInventory.Empty();
      }
      ...
      private bool EmptyBlockInventories(IMyCubeBlock block, IMyInventory dstInventory)
      {
         ...
         if (Mod.Log.ShouldLog(Logging.Level.Verbose)) Mod.Log.Write(Logging.Level.Verbose, "BuildAndRepairSystemBlock {0}: EmptyBlockInventories remainingVolume={1}", Logging.BlockName(_Welder, Logging.BlockNameOptions.None), remainingVolume);
      ...
      private void AppendingCustomInfo(IMyTerminalBlock terminalBlock, StringBuilder details)
      {
         ...
         if ((_Welder.Enabled || MyAPIGateway.Session.CreativeMode) && _Welder.IsWorking && _Welder.IsFunctional)
         {
            ...
            if (State.NeedGrinding && State.InventoryFull) details.Append("Block inventory is full!" + Environment.NewLine);
