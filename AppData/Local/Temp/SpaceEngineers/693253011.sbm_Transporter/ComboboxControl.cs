...
namespace LSE.Control
{
    ...
    {
        public static Dictionary<string, Dictionary<long, List<MyTerminalControlListBoxItem>>> Values =
            new Dictionary<string, Dictionary<long, List<MyTerminalControlListBoxItem>>>();
        public int Size;
            ...
            : base(block, internalName, title)
        {
            ...
            if (Content != null)
            {
                Values[InternalName][block.EntityId] = Content;
            }
            CreateUI();
        }
        ...
        public void FixValues(IMyTerminalBlock block)
        {
            ...
            {
                Values[InternalName] = new Dictionary<long, List<MyTerminalControlListBoxItem>>();
            }

            if (!Values[InternalName].ContainsKey(block.EntityId))
            {
                Values[InternalName][block.EntityId] = new List<MyTerminalControlListBoxItem>();
            }
        }
        ...
        public virtual void FillContent(IMyTerminalBlock block, List<MyTerminalControlListBoxItem> items, List<MyTerminalControlListBoxItem> selected)
        {
            ...
            items.Clear();
            selected.Clear();
            foreach (var value in Values[InternalName][block.EntityId])
            {
                items.Add(value);
            }
        ...
        */
        ...
        public virtual List<string> GetterName(IMyTerminalBlock block)
        {
            ...
            foreach (var index in indices)
            {
                if (index < Values[InternalName][block.EntityId].Count)
                {
                    names.Add(Values[InternalName][block.EntityId][index].Text.String);
                }
            }
        ...
        public virtual List<T2> GetterObjects(IMyTerminalBlock block)
        {
            ...
            foreach (var index in indices)
            {
                if (index < Values[InternalName][block.EntityId].Count)
                {
                    names.Add((T2)Values[InternalName][block.EntityId][index].UserData);
                }
            }
        ...
        public virtual List<int> Getter(IMyTerminalBlock block)
        {
            ...
            if (xmlList != null)
            {
                foreach (var xml in xmlList)
                {
                    ...
                    var index = Values[InternalName][block.EntityId].FindIndex((x) => x.UserData.Equals(obj));
                    if (index >= 0)
                    {
                        indices.Add(index);
                    }
