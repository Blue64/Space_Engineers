namespace JumpExplode
{
	...
	*/
	...
	{
		private const string MOD_NAME = "SEJumpExplode";
		...
		public static DebugLevel LogDebugLevel = DebugLevel.None;
		...
		{
			if (!isRunning) return;
			if (LogDebugLevel == DebugLevel.None) return;
			if (LogDebugLevel == DebugLevel.Custom)
			{
				if (_d == DebugLevel.Custom) Info(msg);
				return;
			}

			if (LogDebugLevel == DebugLevel.Error)
			{
				if (_d == DebugLevel.Error)
					Error(msg);
				return;
			}
			if (LogDebugLevel == DebugLevel.Info)
			{
				if (_d == DebugLevel.Error)
					Error(msg);
			...
			if (LogDebugLevel == DebugLevel.Verbose)
			{
				if (_d == DebugLevel.Error)
					Error(msg);
				if (_d == DebugLevel.Info)
