namespace GasExplode
{
	...
	*/
	...
	{
		private const string MOD_NAME = "SEExplode";
		...
		public static void DebugWrite(DebugLevel _d, string msg)
		{
			...
			if (Core.instance.debug == DebugLevel.None) return;
			if (Core.instance.debug == DebugLevel.Custom)
			{
				if (_d == DebugLevel.Custom) Info(msg);
				return;
			}

			if (Core.instance.debug == DebugLevel.Error)
			{
				if (_d == DebugLevel.Error)
					Error(msg);
				return;
			}
			if (Core.instance.debug == DebugLevel.Info)
			{
				if (_d == DebugLevel.Error)
					Error(msg);
			...
			if (Core.instance.debug == DebugLevel.Verbose)
			{
				if (_d == DebugLevel.Error)
					Error(msg);
				if (_d == DebugLevel.Info)
