...
namespace vorgonian.DisableAllAutomaticJetpackActivation
{
    ...
    {
        private short tick;
        public static bool skipNextActivation = false;
        ...
        protected override void UnloadData()
        {
            try
            {
                if (handlerAdded)
                {
                    characterEntity.OnMovementStateChanged -= characterEntity_OnMovementStateChanged;
                    handlerAdded = false;
                }
            }
            catch (Exception e)
            ...
            try
            {
                ...
                if (!handlerAdded) //only once
                {
                    ...
                    if (camera is IMyCharacter)
                    {
                        ...
                        characterEntity.OnMovementStateChanged += characterEntity_OnMovementStateChanged; //add the handler
                        handlerAdded = true;
                    }
                }
            }
